...
;// CONCATENATED MODULE: ./src/js/modules/TaskOne.js
...
var TaskOne = /*#__PURE__*/function () {
  function TaskOne(element) {
  ...
  _createClass(TaskOne, [{
    ...
    key: "createHtml",
    value: function createHtml(element) {
      var _this = this;
      this.test = element;
      ...
      var button = '<button>Очистить статистику</button>';
      element.insertAdjacentHTML('beforeEnd', button);
      this.button = this.element.querySelector('button');
      this.button.addEventListener('click', function (event) {
        event.preventDefault();
        _this.buttonClearPoints();
      });

      // добавляем ячейки игрового поля
      for (var i = 0; i < Math.pow(this.fieldSize, 2); i += 1) {
        // создаём див
    ...
    key: "goblinActive",
    value: function goblinActive() {
      var _this2 = this;
      this.cells[this.lastCell].classList.remove('active');
      var rand = function rand() {
        var random = Math.floor(Math.random() * Math.pow(_this2.fieldSize, 2));
        if (random === _this2.lastCell) {
          return rand();
        }
        _this2.lastCell = random;
        return random;
      };
      var index = rand();
    ...
    key: "clickCell",
    value: function clickCell(event) {
      event.preventDefault();
      var index = this.cells.indexOf(event.currentTarget);
      var active = this.cells[index].classList.contains('active');
      if (active) {
        this.hits += 1;
    ...
    key: "buttonClearPoints",
    value: function buttonClearPoints() {
      this.pointer.forEach(function (item) {
...
;// CONCATENATED MODULE: ./src/js/modules/TaskTwo.js
function TaskTwo_typeof(obj) { "@babel/helpers - typeof"; return TaskTwo_typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (obj) { return typeof obj; } : function (obj) { return obj && "function" == typeof Symbol && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }, TaskTwo_typeof(obj); }
...
function TaskTwo_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
function TaskTwo_defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, TaskTwo_toPropertyKey(descriptor.key), descriptor); } }
function TaskTwo_createClass(Constructor, protoProps, staticProps) { if (protoProps) TaskTwo_defineProperties(Constructor.prototype, protoProps); if (staticProps) TaskTwo_defineProperties(Constructor, staticProps); Object.defineProperty(Constructor, "prototype", { writable: false }); return Constructor; }
function TaskTwo_toPropertyKey(arg) { var key = TaskTwo_toPrimitive(arg, "string"); return TaskTwo_typeof(key) === "symbol" ? key : String(key); }
function TaskTwo_toPrimitive(input, hint) { if (TaskTwo_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (TaskTwo_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
var TaskTwo = /*#__PURE__*/function () {
  function TaskTwo(element) {
    TaskTwo_classCallCheck(this, TaskTwo);
    this.element = element;
    this.responce = [{
      id: 26,
  ...
  TaskTwo_createClass(TaskTwo, [{
    ...
    value: function createHtml(element) {
      ...
      var button = '<button>Остановить интервал</button>';
      element.insertAdjacentHTML('beforeEnd', button);
      ...
      for (var i = 0; i < responce.length; i += 1) {
        var _responce$i = responce[i],
          id = _responce$i.id,
          title = _responce$i.title,
          year = _responce$i.year,
          imdb = _responce$i.imdb;
        var tr = document.createElement('tr');
        tr.dataset.id = id;
        tr.dataset.title = title;
        tr.dataset.year = year;
        tr.dataset.imdb = imdb;
        tr.innerHTML = "\n          <td>".concat(id, "</td>\n          <td>").concat(title, "</td>\n          <td>(").concat(year, ")</td>\n          <td>imdb: ").concat(imdb.toFixed(2), "</td>\n        ");
        this.table.appendChild(tr);
      }
    }
  }, {
    key: "sortFunc",
    value: function sortFunc(sortingState, sortEl) {
      this.headerClear(); // удаляет стрелочки с заголовков

      // массив сортируемых строк
      var tableRows = Array.from(this.table.querySelectorAll('tr[data-id]'));
      // if (!tableRows) { return; }

      // ищем индекс ячейки загаловка таблицы
      var index = this.th.findIndex(function (i) {
        return i.textContent === sortEl;
      });
      var condition = 'number'; // условие сортировки по умолчанию для number

      // если в строке есть хотя бы один string, то условие сортировки string
      for (var i = 0; i < this.responce.length; i += 1) {
      ...
      if (condition === 'number') {
        if (sortingState === 0) {
          tableRows.sort(function (a, b) {
            return a.dataset[sortEl] - b.dataset[sortEl];
          });
          this.th[index].textContent += " \u2191";
        }
        if (sortingState === 1) {
          tableRows.sort(function (a, b) {
            return b.dataset[sortEl] - a.dataset[sortEl];
          });
          this.th[index].textContent += " \u2193";
        }
      }
      ...
      if (condition === 'string') {
        if (sortingState === 0) {
          tableRows.sort(function (a, b) {
            if (a.dataset[sortEl] > b.dataset[sortEl]) {
              return 1;
            }
            if (a.dataset[sortEl] < b.dataset[sortEl]) {
              return -1;
    ...
    key: "sortID",
    value: function sortID() {
      // сортировка по ID и переключения индикатора сортировки
      this.sortingID = this.sortFunc(this.sortingID, 'id');
    }
  }, {
    key: "sortTitle",
    value: function sortTitle() {
      // сортировка по Title и переключения индикатора сортировки
      this.sortingTitle = this.sortFunc(this.sortingTitle, 'title');
    }
  }, {
    key: "sortYear",
    value: function sortYear() {
      // сортировка по Year и переключения индикатора сортировки
      this.sortingYear = this.sortFunc(this.sortingYear, 'year');
    }
  }, {
    key: "sortImdb",
    value: function sortImdb() {
      // сортировка по Imdb и переключения индикатора сортировки
    ...
    key: "rendering",
    value: function rendering(items) {
    ...
    key: "intervalSorting",
    value: function intervalSorting() {
      if (this.intervalCount > 7) {
        this.intervalCount = 0;
      }
    ...
    key: "buttonSetInterval",
    value: function buttonSetInterval() {
    ...
    key: "eventButton",
    value: function eventButton() {
      var _this = this;
      this.th[0].addEventListener('click', function (evt) {
        evt.preventDefault();
        _this.sortID();
      });
      this.th[1].addEventListener('click', function (evt) {
        evt.preventDefault();
        _this.sortTitle();
      });
      this.th[2].addEventListener('click', function (evt) {
        evt.preventDefault();
        _this.sortYear();
      });
      this.th[3].addEventListener('click', function (evt) {
        evt.preventDefault();
        _this.sortImdb();
      });
      this.button = this.element.querySelector('button');
...
var divForTaskOne = document.getElementById('taskOne');
var taskOne = new TaskOne(divForTaskOne);
taskOne.init();
var divForTaskTwo = document.getElementById('taskTwo');
var taskTwo = new TaskTwo(divForTaskTwo);
taskTwo.init();
